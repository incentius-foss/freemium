      ...
      <div class="tw-grid tw-grid-cols-2 tw-gap-4 tw-px-4 tw-grow tw-pb-4">
        <div class=" tw-flex tw-flex-col tw-gap-4">
          <div class="tw-shrink-0 tw-flex tw-gap-2">
            <q-btn @click="input_text = 'This is a sample string.', convert_option='Encode'" no-caps unelevated size="1.1em" class="tw-rounded-lg tw-bg-white/20 ">
            ...
            <q-btn @click="input_text = 'VGhpcyBpcyBhIHNhbXBsZSBzdHJpbmcu', convert_option='Decode'" no-caps unelevated size="1.1em" class="tw-rounded-lg tw-bg-white/20 ">
              <div class="tw-flex tw-justify-between tw-gap-2 tw-items-center">
                <span>Sample Base64 String</span>
              </div>
              <q-tooltip>
                Add a sample base64 string
              </q-tooltip>
            </q-btn>
          </div>
...
export default defineComponent({
  ...
  methods:{
    ...
    encode(){
      try{
        this.output_text = window.btoa(this.input_text)
      }catch{
    ...
    decode(){
      try{
        this.output_text = window.atob(this.input_text)
      }catch{
        this.output_text = this.input_text
      }
    },
