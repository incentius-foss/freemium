...
- item    : Big Shoes
  quantity: 1`
    ...
    handle_action(){
      try {
        const jsonObj = yaml.load(this.input_text);
        this.output_text = JSON.stringify(jsonObj, null, 2);
      } catch (error) {
        this.$q.notify({
          type: 'negative',
    ...
    download(){
      if(this.output_text){
        let fileToSave = new Blob([this.output_text], {
          type: "application/json;charset=utf-8",
          name: 'converted.yaml.to.json'
        });
        this.saveFile(fileToSave, 'converted.yaml.to.json');
        this.$q.notify({
          type:'positive',
