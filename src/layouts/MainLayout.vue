<template>
  <q-layout view="lHh lpr lFf">
    ...
    >
      <div>
        ...
        <div v-if="!$q.screen.lt.sm" class="my-2 ml-1">
          <q-btn icon="reorder" flat @click="collapse=!collapse;">
            <q-tooltip size="10px">Open the sidebar</q-tooltip>
          </q-btn>
        </div>


        <div :key="index" v-for="(link,index) in getSearchData" class="px-1">
          <a @click="$route.path==link.path ? '' : $router.push(link.path)"
             :class="$route.path==link.path ? 'bg-blue-500 text-white rounded-xl border-gray-700' : 'text-black hover:bg-bl-700 hover:text-white'"
             class="cursor-pointer group flex items-center px-4 py-2 text-sm font-medium"
    ...
    >
        ...
        <div class="overflow-hidden mx-3">
          ...
          <div class="mt-2">
            ...
            >
              <a @click="$route.path==link.path ? '' : $router.push(link.path)"
                 :class="$route.path==link.path ? 'bg-blue-500 text-white rounded-xl border-gray-700' : 'text-black hover:bg-bl-700 hover:text-white'"
                 class="cursor-pointer group flex items-center px-2 py-2 text-sm font-medium"
                 aria-controls="dropdown-example" data-collapse-toggle="dropdown-example">
      ...
      <div class="footer flex justify-center q-mt-md text-center text-body2 text-secondary mb-2">
          <span> Made with
            <span>
              <i class="q-icon text-red notranslate material-icons" aria-hidden="true" role="presentation"
                 style="font-size: 18px;">favorite</i> by
            </span>
              <a href="https://incentius.com" class="text-blue text-bold">Incentius</a>
          </span>
      </div>
